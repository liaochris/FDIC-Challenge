...
sb_percap <- c("est","n1_4","n5_9", "n1_9", "n10_19","n20_49", "n50_99")
...
vars <- c("FIPS", "county", "state", "year", "pop", "jobs", "income_adjinf_percap", "est_percap", "n1_4_percap", "n5_9_percap", "n1_9_percap",
          "n10_19_percap", "n20_49_percap", "n50_99_percap", "farm_jobs_percap", "nonfarm_jobs_percap", "owner_jobs_percap", 
          "farm_inc_adjinf_percap", "nonfarm_inc_adjinf_percap", "owner_inc_adjinf_percap", "businessChange",  "onefourChange", "fivenineChange","onenineChange", "cpi_2018")
...
#1-9 business growth
onenineC <- SB_FDIC1_a[!is.na(SB_FDIC1_a$onenineChange),]
onenine_year <- ifelse(onenineC$year >= 2009, 1, 0)
est_1_9 <- onenineC$onenineChange

#differences on differences regression
median2_ar <- summary(onenineC$asset_ratio)[3]
median2_lr <- summary(onenineC$all_loans_ratio)[3]
median2_ci <- summary(onenineC$ci_loans_ratio)[3]
median2_ci1m <- summary(onenineC$dollarloans_ci_LT1M_ratio)[3]
median2_e <- summary(onenineC$equity_ratio)[3]
median2_nf1m <- summary(onenineC$dollarloans_nonfarm_bus_LT1M_ratio)[3]
median2_na_ar <- summary(onenineC$na_asset_ratio)[3]
median2_na_ci <- summary(onenineC$na_ci_loans_ratio)[3]

#small business change ratio treatment
c19_asset_ratio_treatment<- ifelse(onenineC$asset_ratio > median2_ar, 1, 0)
c19_loan_ratio_treatment <- ifelse(onenineC$all_loans_ratio > median2_lr, 1, 0)
c19_ci_ratio_treatment <- ifelse(onenineC$ci_loans_ratio > median2_ci, 1, 0)
c19_ci1m_ratio_treatment <- ifelse(onenineC$dollarloans_ci_LT1M_ratio > median2_ci1m, 1, 0)
c19_equity_ratio_treatment <- ifelse(onenineC$equity_ratio > median2_e, 1, 0)
c19_nonfarm1m_ratio_treatment <- ifelse(onenineC$dollarloans_nonfarm_bus_LT1M_ratio > median2_nf1m, 1, 0)
c19_na_asset_ratio_treatment <- ifelse(onenineC$na_asset_ratio > median2_na_ar, 1, 0)
c19_na_ci_loan_treatment <- ifelse(onenineC$na_ci_loans_ratio > median2_na_ci, 1, 0)

didreg19_ar <- summary(lm(est_1_9 ~ c19_asset_ratio_treatment * onenine_year))
didreg19_lr <- summary(lm(est_1_9 ~ c19_loan_ratio_treatment * onenine_year))
didreg19_ci <- summary(lm(est_1_9 ~ c19_ci_ratio_treatment * onenine_year))
didreg19_ci1m <- summary(lm(est_1_9 ~ c19_ci1m_ratio_treatment * onenine_year))
didreg19_e <- summary(lm(est_1_9 ~ c19_equity_ratio_treatment * onenine_year))
didreg19_nf1m <- summary(lm(est_1_9 ~ c19_nonfarm1m_ratio_treatment * onenine_year))
didreg19_na_ar <- summary(lm(est_1_9 ~ c19_na_asset_ratio_treatment * onenine_year))
didreg19_ci_loans <- summary(lm(est_1_9 ~ c19_na_ci_loan_treatment * onenine_year))
